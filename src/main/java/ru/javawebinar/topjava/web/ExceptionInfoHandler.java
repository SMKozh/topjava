...
public class ExceptionInfoHandler {
        ...
        if (rootMsg != null) {
            ...
            for (Map.Entry<String, String> entry : CONSTRAINS_I18N_MAP.entrySet()) {
                if (lowerCaseMsg.contains(entry.getKey())) {
                    return logAndGetErrorInfo(req, e, false, VALIDATION_ERROR, messageSourceAccessor.getMessage(entry.getValue(), req.getLocale()));
                }
            }
        }
    ...
    private String[] getDetails(BindException e) {
        return e.getBindingResult().getFieldErrors().stream()
                .map(mes -> mes.getField() + " " + mes.getDefaultMessage())
                .toArray(String[]::new);
    }
}